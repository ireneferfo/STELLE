...
def create_variable_permutations(
    base_formulae: List, nvars: int, existing_formulae: Optional[List] = None
) -> List:
    """
    ...
    Args:
        ...
        nvars: Number of variables to permute over
    ...
    for base_formula in base_formulae:
        permutations = compute_permutations([base_formula], nvars)

        # Filter out duplicates
        new_permutations = [p for p in permutations if str(p) not in existing_strings]
...
def find_complete_permutation_groups(formulae: List, nvars: int) -> dict:
    """
    Find complete groups of variable permutations in a formula set.

    Args:
        formulae: List of formulae to analyze
        nvars: Number of variables in the system
    ...
    for template, group_formulae in template_groups.items():
        ...
        expected_permutations = nvars**num_vars_in_template
...
def from_string_to_formula(st):
    ...
    if root_arity <= 1:
        ...
        if root_op_str.startswith("x"):
            ...
            root_phi = Atom(
                var_index=int(st_split[0][2:]),
                lte=atom_sign,
                threshold=float(st_split[2]),
            )
            return root_phi
        else:
            assert (
                root_op_str.startswith("not")
                or root_op_str.startswith("eventually")
            ...
            if root_op_str == "not":
                ...
                unbound, right_unbound, left_time_bound, right_time_bound = (
                    set_time_thresholds(root_op_str)
                )
                root_phi = Eventually(
                    child=from_string_to_formula(" ".join(current_st)),
                    unbound=unbound,
        ...
        if "(" in current_st:
            ...
            if n_children == 1:
                ...
                var_child_idx = (
                    1 if children_range[0][0] <= 1 else 0
                )  # 0 is left child, 1 is right child
                if children_range[0][0] != 0 and current_st[children_range[0][0] - 1][
                    0:2
                ] in ["no", "ev", "al"]:
                ...
                left_child_str = (
                    current_st[:3]
                    if var_child_idx == 0
                    else current_st[children_range[0][0] : children_range[0][1] + 1]
                )
                right_child_str = (
                    current_st[-3:]
                    if var_child_idx == 1
                ...
                root_op_str = (
                    current_st[children_range[0][1] + 1]
                    if var_child_idx == 1
                    else current_st[children_range[0][0] - 1]
                )
                ...
                assert root_op_str[:2] in ["an", "or", "un"]
                left_child_str = current_st[
                    children_range[0][0] : children_range[0][1] + 1
                ]
                right_child_str = current_st[
                    children_range[1][0] : children_range[1][1] + 1
                ]
        else:
        ...
        if root_op_str == "and":
            root_phi = And(
                left_child=from_string_to_formula(left_child_str),
