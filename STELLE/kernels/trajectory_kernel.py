...
class TrajectoryKernel:
    ...
    def compute_rho_phi(
        ...
        if self.exp_rhotau or self.normalize_rhotau:
            if epsilon is None:
                raise ValueError("epsilon must be provided when exp_rhotau or normalize_rhotau is True")
        ...
        selfk_tau = torch.einsum(
            "ijk,ijk->i", rhos_tau_values, rhos_tau_values
        ) / self.samples
    ...
    def _get_formula_robustness(self, formulae: List, save: bool, parallel_workers: int):
        """Get robustness values for formulae, either computed or cached."""
        if self.phis is None:
            rhos_phi, selfk_phi = self.stl_kernel._compute_robustness_no_time(
    ...
    def _compute_trajectory_similarities(self, trajectories: torch.Tensor, epsilon: float) -> torch.Tensor:
    ...
    def _compute_gaussian_kernel(self, rhos1: torch.Tensor, rhos2: torch.Tensor) -> torch.Tensor:
    ...
    def _compute_cosine_kernel(self, rhos1: torch.Tensor, rhos2: torch.Tensor) -> torch.Tensor:
    ...
    def _compute_linear_kernel(self, rhos1: torch.Tensor, rhos2: torch.Tensor) -> torch.Tensor:
