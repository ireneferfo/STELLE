...
class StlKernel:
    ...
    def __init__(
        ...
        if signals is not None:
            ...
            self.signals = measure.sample(points=points, samples=samples, varn=varn).to(
                self.device
            )
    ...
    def compute_bag(
        self, formulae: List, return_robustness: bool = False, workers: int = 0
    ) -> torch.Tensor:
        """
        ...
        """
        rhos, selfk = self._compute_robustness(formulae, workers)
        kernel_matrix = self._compute_kernel(rhos, rhos, selfk, selfk)
    ...
    def compute_bag_bag(
        ...
        Returns:
            Kernel matrix or tuple with additional robustness data
        """
        rhos1, selfk1 = self._compute_robustness(formulae1, workers)
        rhos2, selfk2 = self._compute_robustness(formulae2, workers)
        kernel_matrix = self._compute_kernel(rhos1, rhos2, selfk1, selfk2)
    ...
    def _compute_robustness(
        self, formulae: List, workers: int
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [
                executor.submit(
                    self._compute_single_rho, formula, self.boolean, self.signals
                )
    ...
    def _compute_single_rho(
        self, formula, boolean: bool, signals: torch.Tensor
    ) -> torch.Tensor:
        ...
        if boolean:
            ...
            rho = formula.quantitative(
                signals, vectorize=self.vectorize, normalize=self.normalize
            )
    ...
    def _compute_kernel(
        ...
        if self.vectorize:
            r1 = rhos1 if self.device == 'mps' else rhos1.double()
            r2 = rhos2 if self.device == 'mps' else rhos2.double()
            ...
            kernel_matrix = torch.mm(rhos1.to(self.device), rhos2.to(self.device).t())
    ...
    def _exponentiate_kernel(
        ...
        selfk = (
            selfk1.pow(2).repeat(1, k2).cpu() + selfk2.pow(2).t().repeat(k1, 1).cpu()
        )
