...
class ModelConfig:
    """Configuration for NoKembModel initialization."""
    ...
    def __init__(
        ...
        initial_attention_scale: float = 1,
        dropout: float = 0.2,
        tune: bool = True,
        ...
        self.initial_attention_scale = initial_attention_scale
...
class ConceptBasedModel(nn.Module):
    ...
    This model uses temporal embeddings and attention mechanisms to classify
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.Gx = config.Gx
        self.attnorm = config.attnorm
        self.weights_flag = config.use_weights
        self.logging = config.logging

        # Initialize parameters
        self._init_learnable_parameters(config)
    ...
    def _init_learnable_parameters(self, config: ModelConfig):
        """Initialize learnable parameters."""
        # Attention scale
        self.attn_scale = nn.Parameter(
            torch.tensor(math.log(config.initial_attention_scale), device=self.device),
        ...
        self.attn_collapse_strength = nn.Parameter(
    ...
    def _init_classifier(self, config: ModelConfig):
    ...
    def update_parameters(
        ...
        initial_attention_scale: Optional[float] = None,
    ):
        ...
        if initial_attention_scale is not None:
            self.attn_scale = nn.Parameter(
                torch.tensor(math.log(initial_attention_scale), device=self.device),
                requires_grad=self.tune,
            )
    ...
    def forward(
        ...
        Returns:
            Tuple of (class_scores, attention, temb, attG_raw, G_phis)
        """
        x = x.to(self.device)
        ...
        # Compute attention
        attention = self._compute_attention(temb)

        # Apply dropout
        attention_dropped = self.dropout(attention)

        # Compute G_phis based on robustness values
        G_phis = self._compute_G_phis_matrix(x, attention, trainingmode)

        # Compute attention-weighted features
        attG_raw = self._compute_weighted_features(attention_dropped, G_phis)

        # Apply output activation and classify
        attG = self.output_activation(attG_raw).to(self.device)
        class_scores = self.classifier(attG.float().to(self.device))

        # Return the class scores, attention weights, temporal embeddings, and attention-weighted G_phi values
        return class_scores, attention, temb, attG_raw, G_phis
    ...
    def _compute_attention(self, temb: torch.Tensor) -> torch.Tensor:
        """Compute attention weights from temporal embeddings."""
        ...
        # Compute attention
        attention = compute_attention_jit(
            temb_normalized, self.attn_scale, self.attnorm
        ).to(self.device)

        return attention

    def _compute_G_phis_matrix(
        self, x: torch.Tensor, attention: torch.Tensor, trainingmode: bool
    ) -> torch.Tensor:
        """Compute G_phis matrix based on configuration."""
        if self.Gx == "robs":
            return self._compute_G_phis_robustness_only(x, attention, trainingmode)
        elif self.Gx:
            return self._compute_G_phis_full(x, attention, trainingmode)
        else:
            return self._compute_G_phis_identity(x)

    def _compute_G_phis_robustness_only(
        self, x: torch.Tensor, attention: torch.Tensor, trainingmode: bool
    ) -> torch.Tensor:
        ...
        if trainingmode:
            ...
            with torch.no_grad():
                G_phis = self._compute_selective_robustness(x, attention)

        # Expand to (batch, phis, num_classes)
        return G_phis.unsqueeze(-1).expand(-1, -1, self.num_classes)

    def _compute_G_phis_full(
        self, x: torch.Tensor, attention: torch.Tensor, trainingmode: bool
    ) -> torch.Tensor:
        """Compute full G_phis with class-specific statistics."""
        if trainingmode:
            return torch.stack(
                [
            ...
            with torch.no_grad():
                robs = self._compute_selective_robustness(x, attention)
                return self._compute_G_phis_from_robs(robs).to(self.device)

    def _compute_G_phis_identity(self, x: torch.Tensor) -> torch.Tensor:
    ...
    def _compute_selective_robustness(
        ...
        """Compute robustness values only for high-attention concepts."""
        att_threshold = torch.quantile(attention, 0.1, dim=1, keepdim=True)
        att_mask = attention > att_threshold
    ...
    def _compute_weighted_features(
        self, attention: torch.Tensor, G_phis: torch.Tensor
    ) -> torch.Tensor:
        """Compute attention-weighted features."""
        # Detach attention to prevent gradient flow
        detached_attention = attention.detach()

        # Compute weighted features and reshape
        attG_raw = (
            (detached_attention.unsqueeze(-1) * G_phis)
    ...
    def train_model(
        ...
        Args:
            ...
            verbose: Verbosity level (0=silent, 1=every epoch, etc.)
            patience: Early stopping patience
            get_robs: Whether to return robustness values
            robs: Pre-computed robustness values
            checkpath: Path for saving checkpoints
            check_frequency: Frequency of checkpoint saving
        ...
        # Prepare robustness information if needed
    ...
    def _load_or_compute_robustness(
        ...
        # Compute robustness if needed
        if self.Gx and self.robs_mean is None:
    ...
    def _compute_and_store_robustness(self, trainloader, robs: Optional[torch.Tensor]):
        ...
        # Compute robustness values if not provided
        if robs is None:
            robs = (
                torch.stack(
                    [
    ...
    def _train_epoch(self, trainloader, optimizer) -> Dict[str, Any]:
        ...
        for trajectories, labels in trainloader:
            ...
            # Forward pass
            class_scores, _, _, _, _ = self(trajectories)
            ...
            # Clamp parameters
            with torch.no_grad():
                self.attn_scale.clamp_(min=-10, max=10)
    ...
    def _log_training_metrics(self, train_metrics: Dict[str, Any], epoch: int):
        ...
        print(f"Collapse strength: {torch.exp(self.attn_collapse_strength).item():.1e}")
        print(
            f"Epsilon control strength: {torch.exp(self.eps_control_strength).item():.1e}"
        )
        print(f"Attention scale: {torch.exp(self.attn_scale).item():.5f}")
    ...
    def validate(self, valloader, by_class_stats: bool = False) -> Dict[str, Any]:
        ...
        Args:
            ...
            by_class_stats: Whether to print class-wise statistics
        ...
        with torch.no_grad():
            for trajectories, labels in valloader:
                ...
                # Forward pass
                class_scores, _, _, _, _ = self(trajectories, trainingmode=False)
    ...
    def get_explanations(
        ...
        with torch.no_grad():
            x = x.to(self.device)
            class_scores, _, _, attGs_raw, _ = self.forward(x, trainingmode=False)
        attGs = self.output_activation(attGs_raw) if norm else attGs_raw
        ...
        if filter_onlycorrect and y_true is not None:
            x, y_pred, attGs, class_scores = self._filter_correct_predictions(
                x, y_true, y_pred, attGs, class_scores
            )
    ...
    def _compute_attributions(
        ...
        if method == "og":
            return self._compute_og_attributions()
        elif method == "random":
    ...
    def _compute_og_attributions(self) -> torch.Tensor:
    ...
    def _compute_gradient_attributions(
        ...
        class ForwardWrapper(nn.Module):
            ...
            def forward(self, x):
                class_scores, _, _, _, _ = self.model.forward(x, trainingmode=False)
                return class_scores
    ...
    def _compute_final_attributions(
        ...
        if attribution_weights.dim() == 2:
            # For og, random, identity methods
            final_matrix = torch.zeros_like(attGs).cpu()
            for i in range(len(attGs)):
                final_matrix[i] = attGs[i].cpu() * attribution_weights[y_pred[i]]
        else:
            # For gradient-based methods
            final_matrix = attGs.cpu() * attribution_weights
...
@torch.jit.script
def compute_attention_jit(
    temb: torch.Tensor, attn_scale: torch.Tensor, attnorm: bool
) -> torch.Tensor:
    """Compute attention weights with optional normalization."""
    scale = torch.exp(attn_scale).clamp(min=1e-6)
    if attnorm:
...
def compute_loss(logits: torch.Tensor, labels: torch.Tensor, model) -> torch.Tensor:
    ...
    # Attention collapse regularization
    if model.attn_scale is not None:
        collapse_strength = torch.exp(model.attn_collapse_strength)
        sigmoid_temp = torch.exp(model.log_sigmoid_temp)
        loss += collapse_strength * torch.sigmoid(-model.attn_scale / sigmoid_temp)
