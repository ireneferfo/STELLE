...
def main():
    ...
    base_path = "tests/results/explanation_operations/"
    ...
    for lr in [1e-4, 1e-5, 1e-6]:
        ...
        for explanation_operation in [None, "max", "mean"]:
            print(
                    f"\n>>>>>>>>>>>>>>>>>>>>> operation = {explanation_operation} >>>>>>>>>>>>>>>>>>>>>\n"
                )
            config = replace(config, explanation_operation=explanation_operation)
            ...
            if local_metrics is None:
                ...
                local_metrics, global_metrics = compute_explanations(
                    args_explanations, explanation_operation=explanation_operation, save = False
                )
